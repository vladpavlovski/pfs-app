...
const MaterialUiSearchBox = ({
  ...
  const initialValue = useSelector(
    state => state.simpleValues[PERSISTENT_SEARCH_VALUE_KEY]
  )
...
MaterialUiSearchBox.propTypes = {
  classes: PropTypes.shape({
    inputInput: PropTypes.any,
    inputRoot: PropTypes.any,
...
const ConnectedSearchBox = connectSearchBox(
  withStyles(styles)(injectIntl(MaterialUiSearchBox))
)
...
const Search = ({ hitComponent, indexName, appID, apiKey }) => {
  ...
  const Hits = ({ hits }) => (
    <List>{hits.map(hit => hitComponent({ hit }))}</List>
  )
  ...
  return (
    <InstantSearch indexName={indexName} searchClient={searchClient}>
      <Activity
        appBarContent={
          <ConnectedSearchBox delay={500} refresh={state.refresh} />
        }
      >
