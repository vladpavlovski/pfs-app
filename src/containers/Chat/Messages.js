...
class Messages extends Component {
  ...
  handleLoadMore = () => {
    ...
    const currentAmount = simpleValues['chat_messages_limit']
      ? simpleValues['chat_messages_limit']
      : pageStep
  ...
  renderList(messages) {
    ...
    return messages.map((row, i) => {
      ...
      const stringDate = values.created
        ? new Date(values.created).toISOString().slice(0, 10)
        : ''
      let dataChanged = false
      let authorChanged = false
      const backgroundColor =
        values.authorUid === auth.uid ? theme.palette.grey[500] : myBColor
